...
(function() {
	...
	var Core = {
		...
		initSlider: function(options){
			var sliders = $(".slider");
			sliders.each(function(i,b) {
				var max = $(b).data('max'), min = $(b).data('min'), parent = $(b).parents('.b-search__main-form-range'),
					min_input = $('.j-min', parent),max_input = $('.j-max', parent);
				$(b).slider({
					range: true,
					animate: true,
					min: min,
					...
					slide: function( event, ui ) {
						$(".ui-slider-handle span.min", this).text(ui.values[0]);
						$(".ui-slider-handle span.max", this).text(ui.values[1]);
					},
				...
				$(".ui-slider-handle:first-of-type",this).append("<span class='min'>"+min+"</span>");
				$(".ui-slider-handle:last-of-type",this).append("<span class='max'>"+max+"</span>");
			});
		},
